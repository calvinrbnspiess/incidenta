<div class="fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
      <form method="post" action="{{ url('vehicles')}}" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        @method('POST')
        @csrf
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h2 class="subtitle">Neues Fahrzeug anlegen</h2>
            ...
            <label class="label" for="radioIdentificationPrefix">
                ...
                <div class="grid lg:grid-flow-col gap-2">
                    <input type="text" class="input-field" name="radioIdentificationPrefix" id="radioIdentificationPrefix" placeholder="Kennung" value="{{ old('radioIdentificationPrefix') }}">
                    <input type="text" class="input-field" name="radioIdentification" id="radioIdentification" placeholder="Rufname" value="{{ old('radioIdentification') }}">
                </div>
            </label>
        </div>
